package com.hmibrahimsarkar.smritilipi.data.local.preferences
...
class ThemePreferencesRepository(private val context: Context) {
    ...
    suspend fun deleteCustomTheme(themeId: String) {
        context.dataStore.edit { preferences ->
            ...
            if (preferences[PreferencesKeys.SELECTED_THEME_ID] == themeId) {
                preferences[PreferencesKeys.SELECTED_THEME_ID] = "smritilipi_classic"
            }
        }
    }
    ...
    val editorTopBarName: Flow<String> = context.dataStore.data.map { preferences ->
        preferences[PreferencesKeys.EDITOR_TOP_BAR_NAME] ?: "স্মৃতিলিপি"
    }

    val authorSignatureName: Flow<String> = context.dataStore.data.map { preferences ->
        preferences[PreferencesKeys.AUTHOR_SIGNATURE_NAME] ?: ""
    }
    ...
    suspend fun setEditorTopBarName(name: String) {
        context.dataStore.edit { preferences ->
            if (name.isBlank() || name == "স্মৃতিলিপি" || name == "Smritilipi") {
