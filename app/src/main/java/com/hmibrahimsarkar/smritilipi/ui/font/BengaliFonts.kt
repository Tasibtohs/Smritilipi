...
data class BengaliFontOption(
    ...
    val fontFamily: FontFamily
        get() = FontLoader.getFontFamily(key)
}
...
object BengaliFonts {
    ...
    val fonts: List<BengaliFontOption> = listOf(
        ...
        BengaliFontOption(
            ...
            category = "আর্টিস্টিক",
            description = "ব্যক্তিগত সাহিত্য ও নোট লেখার জন্য জীবন্ত হাতে লেখা স্টাইল",
    ...
    fun getFontByKey(key: String): BengaliFontOption {
        val mappedKey = when (key.lowercase().trim()) {
            ...
            else -> key.lowercase().trim()
        }
        return fonts.find { it.key == mappedKey }
            ?: fonts.find { it.key == key.lowercase().trim() }
            ?: fonts[0]
    }
}
